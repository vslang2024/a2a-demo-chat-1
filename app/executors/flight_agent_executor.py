...
class FlightAgentExecutor(AgentExecutor):
    ...
    async def execute(
        ...
        with log_context(session_id=session_id, agent="flight_agent"):
            logger.info(
                "[Flight] Session %s: Started processing %s -> %s",
                session_id,
                request.from_city,
                request.to_city,
            )
            ...
            try:
                ...
                await self.redis.log_event("agent_start", "flight_agent", {"request": request.dict(), "session_id": session_id})
                ...
                logger.info("[Flight] Session %s: Completed successfully", session_id)

            except Exception as e:
                logger.error("[Flight] Session %s: Error: %s", session_id, e)
    ...
    async def cancel(
        ...
        with log_context(session_id=session_id, agent="flight_agent"):
            logger.info("[Flight] Session %s: Cancellation requested", session_id)

            # Log cancellation to Redis (preserving original logging pattern)
            await self.redis.log_event(
                "cancel_requested",
                "flight_agent",
                {"session_id": session_id, "message": "Flight agent execution cancelled"},
            )
            ...
            await updater.update_status(
                TaskState.cancelled,
                new_agent_text_message(
                    "Flight agent execution was cancelled",
                    task.context_id,
                    task.id,
                ),
                final=True,
    ...
    async def stream_a2a(self, request: BookingRequest):
        ...
        session_id = str(uuid.uuid4())
        with log_context(session_id=session_id, agent="flight_agent"):
            logger.info("Flight executor streaming for %s -> %s", request.from_city, request.to_city)
